...
chr_rank <- function(chr) {
    ...
    x <- gsub("^CHR", "", x)  # drop leading "chr" for consistent comparison
...
#'
#' @param df Data frame containing marker information.
...
#'   or removed.
#'
#' @seealso \code{\link{genoprobs_sync_markers}} (uses this for map order)
#'
#' @export
markers_sort <- function(df, chr_col = "chr", pos_col = "pos", marker_col = "marker") {
