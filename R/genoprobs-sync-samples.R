...
#'
#' @seealso \code{\link{genoprobs_correlate}}, \code{\link{genoprobs_sync_markers}}
#'
#' @export
genoprobs_sync_samples <- function(genoprobs_1, genoprobs_2, sort = TRUE) {
    ...
    if(!length(common_chr)) stop("No overlapping chromosomes between the two genoprobs objects.")
    ...
    if(!length(common)) stop("No overlapping samples between the two genoprobs objects (after requiring samples to appear on all chromosomes).")
    ...
    subset_to <- function(gp, samples) {
        ...
        for(chr in common_chr) {
            ...
            if(anyNA(i)) stop("Sample missing on chromosome ", chr, "; ensure samples appear on all chromosomes.")
